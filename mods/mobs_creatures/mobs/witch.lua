...
mobs:register_mob("mobs_creatures:witch", {
	type = "monster",
	hp_min = 20,
	hp_max = 20,
	...
	arrow = "mobs_creatures:witch_curse",
	shoot_interval = 2,
	dogshoot_switch = 2,
	dogshoot_count = 0,
	...
	drops = {
		...
		{name = "gadgets_consumables:potion_mana_regen_02", chance = 10, min = 1, max = 1},
                ...
                {name = "halloween:mask_cat", chance = 100, min = 1, max = 1},
                {name = "halloween:suit_harry", chance = 100, min = 1, max = 1},
...
-- Witch's Arrow
mobs:register_arrow(":mobs_creatures:witch_curse", {
	visual = "sprite",
	visual_size = {x = 1, y = 1},
...
-- spawn egg
mobs:register_egg("mobs_creatures:witch", "Witch Spawn Egg", "mobs_creatures_witch_arrow.png", 1)

-- Witches will only spawn between (Oct 20 - Nov 3).
local date = os.date("*t")
if (date.month == 10 and date.day >= 20) or (date.month == 11 and date.day <= 3) then
        -- World spawning parameters for the witch.
                mobs:spawn_specific("mobs_creatures:witch", {"group:cracky", "group:crumbly", "group:shovely", "group:pickaxey"}, {"air"}, 0, 5, 240, 3000, 2, -30912, 30192, false)
else
        return
end
