...
mobs:register_mob("mobs_creatures:wolf", {
	type = "animal",
	hp_min = 10,
	hp_max = 10,
	passive = false,
	...
	visual = "mesh",
	mesh = "mobs_creatures_wolf.b3d",
	textures = {
		{"mobs_creatures_wolf.png"},
	},
	visual_size = {x=3, y=3},
	makes_footstep_sound = true,
	sounds = {
		random = "mobs_creatures_wolf_random",
		damage = "mobs_creatures_wolf_pain",
		death = "mobs_creatures_wolf_death",
		attack = "mobs_creatures_wolf_warcry",
		distance = 16,
	},
	pathfinding = 1,
	floats = 1,
	...
	lava_damage = 4,
	light_damage = 0,
	follow = {"mobs_creatures:bone", "mobs_creatures:rotten_flesh", "group:food_meat", "group:food_meat_raw", "group:food_chicken", "group:food_chicken_raw", "group:food_fish", "group:food_fish_raw"},
	on_rightclick = function(self, clicker)
		-- Try to tame wolf (intentionally does NOT use mobs:feed_tame)
		local tool = clicker:get_wielded_item()

		local dog, ent
		if tool:get_name() == "mobs_creatures:bone" then
...
-- DOG (TAMED WOLF)
mobs:register_mob("mobs_creatures:dog", {
        type = "animal",
        hp_min = 20,
        hp_max = 20,
        passive = true,
        ...
        mesh = "mobs_creatures_wolf.b3d",
	textures = {
		{"mobs_creatures_wolf_tame.png^(mobs_creatures_wolf_collar.png^[colorize:#0000BB:192)"},
        },
        visual_size = {x=3, y=3},
        makes_footstep_sound = true,
        sounds = {
	...
	on_rightclick = function(self, clicker)
		...
		elseif item:get_name() == "mobs_creatures:bone" then
	                ...
	                if eatable > 0 then
	                        hp_add = eatable
	                elseif item:get_name() == "mobs_creatures:rotten_flesh" or minetest.get_item_group(item:get_name(), "food_meat_raw") == 1 or minetest.get_item_group(item:get_name(), "food_meat") == 1 or minetest.get_item_group(item:get_name(), "food_chicken") == 1 or minetest.get_item_group(item:get_name(), "food_chicken_raw") == 1 or minetest.get_item_group(item:get_name(), "food_fish_raw") == 1 or minetest.get_item_group(item:get_name(), "food_fish") == 1 then
	                        hp_add = 4
	                else
	                        hp_add = 4
	                end
...
--		return {"mobs_creatures_wolf_tame.png^(mobs_creatures_wolf_collar.png^[colorize:"..colors[color]..":192)"}
--	else
--		return nil
--	end
--end
...
mobs:spawn_specific("mobs_creatures:wolf", {"default:dirt_with_grass", "default:dirt_with_rainforest_litter", "default:dirt", "default:dirt_with_snow", "default:snow", "default:snowblock", "ethereal:prairie_dirt"}, {"air"}, 1, 8, 240, 6000, 2, 3, 100)

-- Register Spawn Egg
mobs:register_egg("mobs_creatures:wolf", "Wolf Spawn Egg", "wool_white.png", 1)
