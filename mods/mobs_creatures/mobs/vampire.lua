mobs:register_mob('mobs_creatures:vampire', {
	type = "monster",
	passive = false,
	...
	{name = "mobs_creatures:bone", chance = 2, min = 0, max = 1},
        ...
        {name = "halloween:suit_vampire", chance = 100, min = 1, max = 1},
	},
	lava_damage = 5,
	water_damage = 2,
	light_damage = 10,
...
--Spawn Egg
mobs:register_egg("mobs_creatures:vampire", "Vampire Spawn Egg", "wool_black.png", 1)

local date = os.date("*t")
if (date.month == 10 and date.day >= 20) or (date.month == 11 and date.day <= 3) then
	--Spawn Functions
	mobs:spawn_specific("mobs_creatures:vampire", {"group:cracky", "group:crumbly", "group:shovely", "group:pickaxey"}, {"air"}, 0, 6, 480, 3000, 3, -30192, 30912, false)
else
        return
end
