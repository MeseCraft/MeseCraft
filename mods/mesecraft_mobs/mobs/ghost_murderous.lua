...
mobs:register_mob("mesecraft_mobs:murderous_ghost", {
	type = "monster",
	passive = false,
	group_attack = true,
	...
	do_custom = function(self, pos)	-- ghosts will stay to haunt if there is no proper burial (gravestone) or if bones are still present.
		...
		elseif bonecheck == nil and gravecheck ~= nil then	-- ghost can pass on
			minetest.sound_play({name = "mesecraft_mobs_ghost_death", gain = 1.0}, {pos=checkpos, max_hear_distance = 12})
		...
		if minetest.get_node(checkpos).name ~= "air" then
			self.collisionbox = {0,0,0,0,0,0}
                ...
                minetest.add_particlespawner({
                        ...
                        "mesecraft_mobs_common_shadow.png" --texture
	                })
	end,
})

--Spawn Eggs
mobs:register_egg("mesecraft_mobs:murderous_ghost", "Murderous Ghost Spawn Egg", "wool_black.png", 1)

--Spawn Functions
mobs:spawn_specific("mesecraft_mobs:murderous_ghost", {"mesecraft_bones:bones"}, {"air"}, 0, 7, 240, 300, 2, -30912, -500)
