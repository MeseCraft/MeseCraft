...
-- define namespace.
christmas_holiday_pack.open_present = function (color)
    	local on_use = function(itemstack, user, pointed_thing)
    		-- Safety check to ensure at least 1 item exists to be given
    		if christmas_holiday_pack["christmas_common_item_pool"][1] == nil then
    			return itemstack
    		end
    		...
    		for i = 1, num_of_unique_drops, 1 do
    			...
    			if color_exclusive_chance == 1 and christmas_holiday_pack[color .. "_item_pool"][1] ~= nil then
    				table.insert(item_drops, christmas_holiday_pack[color .. "_item_pool"][math.random(#christmas_holiday_pack[color .. "_item_pool"])] .. " " .. stack_num)
    			else
    				table.insert(item_drops, christmas_holiday_pack["christmas_common_item_pool"][math.random(#christmas_holiday_pack["christmas_common_item_pool"])] .. " " .. stack_num)
    			end
    		end
    		...
    		-- Play the sound
    		minetest.sound_play("christmas_holiday_pack_sleighbell", {pos=pos, max_hear_distance = 2, gain = 1.0,})
