...
function drinks.juice_press_formspec(action_desc)
  ...
  local def = minetest.registered_items[drinks.tube] or minetest.registered_nodes[drinks.tube]
  local tubename = def.description
  return
  'size[8,7]'..
  'label[1.5,0;'..F(action_desc)..']' ..
  'label[4.2,.75;<- '..F(S('Put fruit here'))..']'..
  'label[4.2,1.75;<- '..F(S('Put container here'))..']'..
  'label[3.2,2.4;'..F(S('(Use @1 to drain into container below)', tubename))..']'..
  'label[0.2,1.8;'..F(cup_req)..' fruits to a glass,]'..
  'label[0.2,2.1;'..F(bottle_req)..' fruits to a bottle,]'..
  'label[0.2,2.4;'..F(bucket_req)..' fruits to a bucket.]'..
  'button[0.5,1;2.5,1;press;'..F(S('Start Juicing'))..']'..
